	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Sudah Absen Masuk';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Selamat Datang '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Sudah Absen Pulang';
					}
					else{
						
						$sql = "UPDATE attendance SET time_out = NOW() WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
							$output['message'] = 'Terimakasih Untuk Harinya '.$row['firstname'].' '.$row['lastname'];
			...
			$output['error'] = true;
			$output['message'] = 'ID Karyawan tidak ditemukan';
		}
